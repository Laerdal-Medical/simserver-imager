...
# Set up icon resources We use the .icon file from Icon Composer for dark mode
# support (macOS Tahoe+) Plus .icns as fallback for older macOS versions
set_source_files_properties("icons/laerdal-simserver-imager.icns"
...
if(EXISTS "${ICON_COMPOSER_FILE}" AND NOT BUILD_CLI_ONLY)
  ...
  message(
    STATUS "Will include .icon file for dark mode support on macOS Tahoe+")

  # Copy the .icon bundle to Resources as AppIcon.icon The .icon file is a
  # bundle (directory) so we track icon.json for changes
  ...
  add_custom_command(
    ...
    COMMAND ${CMAKE_COMMAND} -E copy_directory "${ICON_COMPOSER_FILE}"
            "${APPICON_OUTPUT}"
