...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--log-file") == 0 && i + 1 < argc) {
            ...
            if (g_logFile) {
#ifdef Q_OS_UNIX
                fprintf(g_logFile, "\n=== Laerdal SimServer Imager started (PID %d, EUID %d) ===\n",
                        getpid(), geteuid());
#else
                fprintf(g_logFile, "\n=== Laerdal SimServer Imager started ===\n");
#endif
                fflush(g_logFile);
    ...
    app.setOrganizationName("Laerdal Medical");
    ...
    app.setDesktopFileName("com.laerdal.simserver-imager");
    ...
    parser.setApplicationDescription("Laerdal SimServer Imager GUI");
    ...
    if (!posArgs.isEmpty())
    {
                 ...
                 firstPos.endsWith(".json", Qt::CaseInsensitive))
        {
            ...
            if (fi.isFile()) {
                ...
                cerr << "Manifest file not found: " << firstPos << endl;
                 ...
                 firstPos.endsWith(".spu", Qt::CaseInsensitive))
        {
        ...
        else
        {
            ...
            cerr << "Supported file types: .wic, .wic.xz, .wic.gz, .wic.zst, .wic.bz2, .vsi, .spu, ." MANIFEST_EXTENSION ", .json" << endl;
        }
    }
    ...
    if (hasPermissionIssue)
    {
        ...
        QString header = QObject::tr("Laerdal SimServer Imager requires elevated privileges to write to storage devices.");
