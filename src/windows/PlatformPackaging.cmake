...
if(CMAKE_STRIP AND CMAKE_BUILD_TYPE MATCHES
...
if(IMAGER_SIGNED_APP)
  ...
  add_custom_command(
    TARGET ${PROJECT_NAME}
    POST_BUILD
    COMMAND "${SIGNTOOL}" sign /tr http://timestamp.digicert.com /td sha256 /fd
            sha256 /a "${CMAKE_BINARY_DIR}/${PROJECT_NAME}.exe")

  add_custom_command(
    TARGET ${PROJECT_NAME}
    POST_BUILD
    COMMAND "${SIGNTOOL}" sign /tr http://timestamp.digicert.com /td sha256 /fd
            sha256 /a "${CMAKE_BINARY_DIR}/laerdal-imager-callback-relay.exe")
endif()
...
add_custom_command(
  ...
  POST_BUILD
  COMMAND
    ${CMAKE_COMMAND} -E copy "${CMAKE_BINARY_DIR}/${PROJECT_NAME}.exe"
...
add_custom_command(
  ...
  POST_BUILD
  COMMAND
    "${WINDEPLOYQT}" --no-translations --no-widgets --skip-plugin-types
    qmltooling --exclude-plugins qtiff,qwebp,qgif --no-quickcontrols2fusion
    --no-quickcontrols2fusionstyleimpl --no-quickcontrols2universal
    --no-quickcontrols2universalstyleimpl --no-quickcontrols2imagine
    --no-quickcontrols2imaginestyleimpl
    --no-quickcontrols2fluentwinui3styleimpl
    --no-quickcontrols2windowsstyleimpl --verbose 2 --qmldir
    "${CMAKE_CURRENT_SOURCE_DIR}"
    "${CMAKE_BINARY_DIR}/deploy/${PROJECT_NAME}.exe")
...
if(ENABLE_INNO_INSTALLER)
  ...
  configure_file(
    ...
    "${CMAKE_CURRENT_BINARY_DIR}/laerdal-simserver-imager.iss" @ONLY)

  find_program(
    INNO_COMPILER
    NAMES iscc ISCC "iscc.exe"
  ...
  if(INNO_COMPILER)
    if(IMAGER_SIGNED_APP)
      add_custom_target(
        ...
        COMMAND
          ...
          "/DSIGNING_ENABLED"
          "/Ssign=${SIGNTOOL} sign /tr http://timestamp.digicert.com /td sha256 /fd sha256 /a $p"
    ...
    message(
      STATUS "Added 'inno_installer' target to build the Inno Setup installer")
  else()
    message(
      WARNING
        "Inno Setup compiler not found. Install Inno Setup from https://jrsoftware.org/isinfo.php"
    )
  endif()
else()
  if(EXISTS
    ...
    configure_file(
      "${CMAKE_CURRENT_SOURCE_DIR}/windows/laerdal-simserver-imager.nsi.in"
      "${CMAKE_CURRENT_BINARY_DIR}/laerdal-simserver-imager.nsi" @ONLY)
  else()
    message(
      STATUS "NSIS template not found; skipping NSIS installer configuration")
  endif()
endif()
...
configure_file(
  ...
  "${CMAKE_CURRENT_BINARY_DIR}/laerdal-simserver-imager.rc" @ONLY)

# Manifest generation and copying
configure_file(
  "${CMAKE_CURRENT_SOURCE_DIR}/windows/laerdal-simserver-imager.manifest.in"
  "${CMAKE_CURRENT_BINARY_DIR}/laerdal-simserver-imager.manifest" @ONLY)
