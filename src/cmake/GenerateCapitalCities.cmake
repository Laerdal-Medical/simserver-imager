...
# GenerateCapitalCities.cmake CMake script to download capital cities data from
# REST Countries API and generate a mapping file with capital cities, country
# codes, timezones, and keyboard layouts. Falls back to capital-cities.txt in
# source tree if download fails.
#
# Called with: -DAPI_URL=<url> -DOUTPUT_FILE=<file> -DSOURCE_DIR=<dir>
# -DKEYMAP_FILE=<file>
...
if(dl_error)
  ...
  if(EXISTS "${FALLBACK_FILE}")
    ...
    message(
      FATAL_ERROR "No fallback capital cities file found at ${FALLBACK_FILE}")
  endif()
endif()
...
# The JSON is an array of objects, we need to split it properly First, remove
# the outer array brackets
...
function(get_default_language country_code result_var)
  ...
  # Language mappings
  if(country_code STREQUAL "ES"
     OR country_code STREQUAL "MX"
     OR country_code STREQUAL "AR"
     OR country_code STREQUAL "CO"
     OR country_code STREQUAL "PE"
     OR country_code STREQUAL "VE"
     OR country_code STREQUAL "CL"
     OR country_code STREQUAL "EC"
     OR country_code STREQUAL "GT"
     OR country_code STREQUAL "CU"
     OR country_code STREQUAL "BO"
     OR country_code STREQUAL "DO"
     OR country_code STREQUAL "HN"
     OR country_code STREQUAL "PY"
     OR country_code STREQUAL "SV"
     OR country_code STREQUAL "NI"
     OR country_code STREQUAL "CR"
     OR country_code STREQUAL "PA"
     OR country_code STREQUAL "UY")
    set(lang "Spanish")
  elseif(
    country_code STREQUAL "FR"
    OR country_code STREQUAL "BE"
    OR country_code STREQUAL "CH"
    OR country_code STREQUAL "LU"
    OR country_code STREQUAL "MC")
    set(lang "French")
  ...
  elseif(country_code STREQUAL "BD")
    set(lang "Bengali")
  elseif(country_code MATCHES
         "^(SA|AE|EG|IQ|JO|KW|LB|OM|QA|SY|YE|BH|DZ|MA|TN|LY|SD)$")
    set(lang "Arabic")
  endif()
...
# Parse JSON manually (CMake doesn't have built-in JSON parser in 3.22) We'll
# extract country data using regex
set(output_content "# Capital Cities Database\n")
set(output_content
    "${output_content}# Format: CityName|CountryName|CountryCode|Timezone|Language|KeyboardLayout\n"
)
set(output_content
    "${output_content}# Generated from REST Countries API (https://restcountries.com/)\n"
)
set(output_content
    "${output_content}# Sorted alphabetically by capital city name\n")
...
foreach(country_obj ${country_objects})
  ...
  # Extract fields using improved regex Country code (cca2)
  ...
  list(
    APPEND
    all_entries
    "${capital}|${country_name}|${country_code}|${timezone}|${language}|${keyboard}"
  )
endforeach()
...
message(
  STATUS "Generated ${num_entries} capital city entries into ${OUTPUT_FILE}")
