...
OUTPUT_FILE="$PWD/Laerdal_SimServer_Imager-${GIT_VERSION}-embedded-${ARCH}.AppImage"
...
echo "Building laerdal-simserver-imager for embedded $ARCH..."
...
cp "debian/com.laerdal.simserver-imager.desktop" "$APPDIR/usr/share/applications/com.laerdal.simserver-imager-embedded.desktop"
# Update the desktop file for embedded use (preserve %F for file arguments)
sed -i 's|Name=.*|Name=Laerdal SimServer Imager (Embedded)|' "$APPDIR/usr/share/applications/com.laerdal.simserver-imager-embedded.desktop"
sed -i 's|Comment=.*|Comment=Laerdal SimServer Imager for embedded systems|' "$APPDIR/usr/share/applications/com.laerdal.simserver-imager-embedded.desktop"
sed -i 's|Exec=.*|Exec=laerdal-simserver-imager-embedded %F|' "$APPDIR/usr/share/applications/com.laerdal.simserver-imager-embedded.desktop"
...
exec "${HERE}/usr/bin/laerdal-simserver-imager" "$@"
EOF
...
<fontconfig>
    ...
    <cachedir>/tmp/fontconfig-cache-laerdal-imager</cachedir>
</fontconfig>
FONTCONFIG_EOF
...
rm -f "$PWD/laerdal-simserver-imager-embedded.AppImage"
rm -f "$PWD/laerdal-simserver-imager-embedded-$ARCH.AppImage"
...
if [ -n "$LINUXDEPLOY" ] && [ -f "$LINUXDEPLOY" ]; then
    ...
    LD_LIBRARY_PATH="$QT_DIR/lib:$LD_LIBRARY_PATH" "$LINUXDEPLOY" --appdir="$APPDIR" \
        --desktop-file="$APPDIR/usr/share/applications/com.laerdal.simserver-imager-embedded.desktop" \
        --output=appimage \
    ...
    # linuxdeploy creates: Laerdal_SimServer_Imager_(Embedded)-${ARCH}.AppImage (based on Name= in desktop file)
    LINUXDEPLOY_OUTPUT="Laerdal_SimServer_Imager_(Embedded)-${ARCH}.AppImage"
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    # Primary symlink matches debian/laerdal-simserver-imager-embedded.install expectations
    DEBIAN_SYMLINK="$PWD/laerdal-simserver-imager-embedded.AppImage"
    ...
    ARCH_SYMLINK="$PWD/laerdal-simserver-imager-embedded-$ARCH.AppImage"
